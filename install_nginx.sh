...
wget -O pcre.tar.gz ftp://ftp.csx.cam.ac.uk/pub/software/programming/pcre/pcre-8.39.tar.gz \
	&& mkdir pcre \
	&& tar zxf pcre.tar.gz -C pcre --strip-components=1


#download zlib library
wget -O zlib.tar.gz http://zlib.net/zlib-1.2.8.tar.gz \
	&& mkdir zlib \
	&& tar zxf zlib.tar.gz -C zlib --strip-components=1

 
#download openssl
wget -O openssl.tar.gz https://www.openssl.org/source/openssl-1.0.2h.tar.gz \
	&& mkdir openssl \
...
Source-Url: https://github.com/gsterjov/openshift-nginx-cartridge/archive/master.zip
Id: 57b7bf3689f5cf80240000ca
EOF
